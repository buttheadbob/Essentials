...
public class RanksAndPermissionsModule {
    ...
    public void UpdateRankObject(RankData obj) {
        var objectToReplace = Ranks.First(i => i.RankName == obj.RankName);
        var index = Ranks.IndexOf(objectToReplace);
        if (index != -1)
            Ranks[index] = obj;
        SaveRankData();
    }
    ...
    {
        Dictionary<string, List<string>> InheritedPerms = [];
        ...
        if (Account is null) return hasPermission;
    ...
    public void RegisterInheritedRanks(IPlayer player) {
        ...
        if (MainRank is null) return;
        if (!PlayersInheritedRanksStore.ContainsKey(steamID)) {
            List<RankData> ListRanks = [] ;
        ...
        string Ranks = $"{MainRank.RankName},";                                     // <-- Is this supposed to hide the base field?
