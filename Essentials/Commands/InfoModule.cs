...
public class InfoModule:CommandModule
{
    ...
    {
        IChatManagerServer? c = EssentialsPlugin.Instance!.Torch.CurrentSession?.Managers?.GetManager<IChatManagerServer>();
        if (c is not null)
    ...
    {
        ObservableCollection<InfoCommand>? infoCommands = EssentialsPlugin.Instance!.Config!.InfoCommands;
        if (infoCommands.Count == 0)
            return;

        InfoCommand? c = infoCommands.FirstOrDefault(i => i.Command.Equals(msg.Message));
        if (c is null)
            return;

        if (msg.AuthorSteamId is null)
            return;
