...
{
    private static EssentialsConfig Config => EssentialsPlugin.Instance!.Config!;
    ...
    {
        AutoCommand? command = EssentialsPlugin.Instance.Config.AutoCommands.FirstOrDefault(c => c.Name.Equals(name));
        if (command == null)
        {
    ...
    {
        AutoCommand? command = EssentialsPlugin.Instance.Config.AutoCommands.FirstOrDefault(c => c.Name.Equals(commandName));
        if (command == null)
        {
    ...
    {
        List<AutoCommand> commands = new (EssentialsPlugin.Instance.Config.AutoCommands.Where(x=>x.IsRunning()));
        ...
        AutoCommand? command = commands[commandNumber - 1];
        if (command == null)
        {
    ...
    public void ListAuto()
    {
        ...
        {
            ModCommunication.SendMessageTo(new DialogMessage("Current AutoCommands",$"Found {Config.AutoCommands.Count} Commands",sb.ToString()), Context.Player.SteamUserId);
        }
    }
    ...
    {
        var commands = new List<AutoCommand>(Config.AutoCommands.Where(x=>x.IsRunning()));
    ...
    private static IChatManagerServer ChatManager => EssentialsPlugin.Instance.Torch.CurrentSession.Managers.GetManager<IChatManagerServer>();
    private readonly List<ulong> _removeCache = [];
    ...
    public void MuteUser(string user, int timeout = 0)
    {
        ...
        IMyPlayer? p = Utilities.GetPlayerByNameOrId(user);
        if (p == null)
        {
        ...
        Context.Respond(!res ? $"Failed to mute user {user}. They are already muted." : $"Muted user {p.DisplayName}");
    ...
    private void _muteTimer_Elapsed(object sender, ElapsedEventArgs e)
    {
        lock (_muted)
        {
            foreach (KeyValuePair<ulong, DateTime> p in _muted)
            {
            ...
            foreach (ulong r in _removeCache)
            {
                _muted.Remove(r);
            }
            _removeCache.Clear();
    ...
    public void UnmuteUser(string user)
    {
        ...
        Context.Respond(!res ? $"Failed to unmute user {user}. They are not muted." : $"Unmuted user {p.DisplayName}");
    }

    [Command("list mute", "Lists all muted users, an their timeout, if applicable.")]
    public void ListMute()
    {
        ...
        foreach (ulong m in ChatManager.MutedUsers)
        {
            string? s = MySession.Static.Players.TryGetIdentityNameFromSteamId(m);
    ...
    [Command("give", "Insert an item with a specific quantity into a players inventory")]
    [Permission(MyPromoteLevel.Admin)]
    public void give(string playerName, string itemType, string item, int quantity) {
        ...
        if (playerName != "*") 
        {
            IMyPlayer? p = Utilities.GetPlayerByNameOrId(playerName);
            if (p is null) {
        ...
        else 
        {
            foreach (MyPlayer? p in MySession.Static.Players.GetOnlinePlayers()) {
                ...
                IMyPlayer? player = Utilities.GetPlayerByNameOrId(p.DisplayName);
