...
namespace Essentials.Commands
{
    ...
    public class BlocksModule : CommandModule
    {
        ...
        private void ProcessBlockCommand(BlockAction action)
        {
            ...
            if (conditionArgs.Count > 0)
            {
                grids = ConditionsChecker.ScanConditions(Context, conditionArgs);
            }
            else
            {
                // No conditions, process all grids
                grids = MyEntities.GetEntities().OfType<MyCubeGrid>().Where(x => x.Projector == null);
            }
