...
public class BlocksModule : CommandModule
{
    ...
    {
        int count = 0;
        foreach (MyCubeGrid? grid in MyEntities.GetEntities().OfType<MyCubeGrid>().Where(x => x.Projector == null))
        {
            foreach (MyFunctionalBlock? block in grid.GetFatBlocks().OfType<MyFunctionalBlock>())
            {
