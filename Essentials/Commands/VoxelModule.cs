...
public class VoxelModule : CommandModule
{
    ...
    public void ResetAll(bool deleteStorage = false)
    {
        ...
        foreach (var map in voxelMaps)
        {
            try
            {
                if (map?.StorageName is null || map.Storage.DataProvider is null)
                    continue;
    ...
    public void CleanupAsteroids(bool deleteStorage = false)
    {
        ...
        foreach (var map in voxelMaps)
        {
            try
            {
                ...
                if (nearEntities.Any(e => e is MyCubeGrid or MyCharacter))
                    continue;
    ...
    public void ResetPlanet(string planetName)
    {
        ...
        switch (maps.Count)
        {
            ...
            case 1:
                var map = maps[0];
                map.Storage.Reset(MyStorageDataTypeFlags.All);
                ModCommunication.SendMessageToClients(new VoxelResetMessage([map.EntityId]));
    ...
    [Command("reset area", "Resets voxel damage in specified radius from player")]
    [Permission(MyPromoteLevel.Admin)]
    public void ResetVoxelArea(float Radius)
    {
        if (Context.Player == null)
        {
            Context.Respond("Invalid command input! Must be in-game!");
            return;       
        }

        if (Radius <= 0)
        {
            Context.Respond("Invalid radius!");
            return;
        }
    ...
    {
        Vector3D ResetTarget = new (X, Y, Z);
        if (Radius <= 0)
        {
        ...
        Context.Respond(ResetVoxelInArea(ResetTarget, Radius) 
            ? "Voxel reset complete!" 
            : "Couldn't reset voxels! Check log for more information!");
    }
    ...
    public static bool ResetVoxelInArea(Vector3D Center, float Radius, bool UpdatePlayers = true)
    {
        try
        {
            ...
            MethodInfo? m_methodImp = typeof(MyVoxelBase).GetMethod("PerformVoxelOperationSphere_Implementation", BindingFlags.Instance | BindingFlags.NonPublic);
            if (m_methodImp is null)
            {
                _log.Error("Couldn't find PerformVoxelOperationSphere_Implementation!");
                return false;           
            }

            foreach (MyVoxelBase? voxelMap in Maps)
            {
                if (voxelMap is null || voxelMap.MarkedForClose)
                    continue;
                ...
                if (UpdatePlayers)
                    Events.RaiseEvent<MyVoxelBase, Vector3D, float, byte, OperationType>(voxelMap.RootVoxel, m_methodImp, shape.Center, shape.Radius, 0, OperationType.Revert);
    ...
    private class LockToken : IDisposable
    {
        public LockToken()
        {
            if (GeneratorInstance != null) _deletingSet(GeneratorInstance, true);
        }

        public void Dispose()
        {
            if (GeneratorInstance != null) _deletingSet(GeneratorInstance, false);
        }
    }
}